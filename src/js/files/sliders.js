...
function initSliders() {
			...
			*/
			on: {
				init: function(swiper){
					const allSlide=document.querySelector('.fraction-controll__all');
					allSlide.innerHTML = swiper.slides.length < 10 ? `0${swiper.slides.length}` : swiper.slides.length;
				},
				slideChange: function(swiper){
					const currentSlide=document.querySelector('.fraction-controll__current');
					currentSlide.innerHTML = swiper.realIndex < 10 ? `0${swiper.realIndex + 1}` : swiper.realIndex + 1;
					console.log(swiper)
				}
			}
		});
	...
	if (document.querySelector('.thumbs-product')) {
		const thumbsSlider = new Swiper('.thumbs-product', {
			// Подключаем модули слайдера
			// для конкретного случая
			modules: [Navigation,Pagination,Autoplay],
			// effect: 'fade',
